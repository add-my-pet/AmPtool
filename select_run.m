%% select_mydata
% gets list of species that belongs to a taxon and has a specified string in its run-file

%%
function [species, nm, sel] = select_run(varargin)
% created 2024/12/17 by  Bas Kooijman

%% Syntax
% [species, nm, sel]  = <../select_run.m *select_run*> (varargin) 

%% Description
% gets all species in the add_my_pet collection with run files that contain a character string.
%
% Input:
%
...
% Output:
% 
% * species: cell string with all species in the add_my_pet collection that belong to that taxon and have a run-file that contains str
...
%% Example of use
% nm = select_run(select,'''results_output'', 3'); diff = setdiff(select,nm)
