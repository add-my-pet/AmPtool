...
%%
function [nm, n_spec] = prt_SpecPerFam(taxon)
  % created by Bas Kooijman 2023/01/21
  
  %% Syntax 
  % [nm, n_spec] = <../prt_SpecPerFam.m *prt_SpecPerFam*>(taxon)
  
  %% Description
  % Print list of families with number of AmP entries for a taxon to SpecPerFam.html in local directory and open it in browser. 
  ...
  % prepare for writing SpecPerFam.html
  fid_SpG = fopen('SpecPerFam.html', 'w+'); % open file for writing, delete existing content
  
  % write header for SpecPerFam.html :
  fprintf(fid_SpG, '<!DOCTYPE html>\n');
  fprintf(fid_SpG, '<html>\n');
  fprintf(fid_SpG, '<head>\n');
  fprintf(fid_SpG, '  <title>SpecPerFam</title>\n');
  ...
  fprintf(fid_SpG, '        <ul id="fam">\n');
  for i = 1:n_fam % scan all families
  ...
  fprintf(fid_SpG, '          ul = document.getElementById("fam");\n');
