...
if isempty(strfind(url,'Search results')) % single result
  i_0 = strfind(url,'avibaseid=') + 10; if isempty(i_0); id = []; return; end
  i_1 = strfind(url(i_0(1):end),'&') + i_0(1) - 2; id = url(i_0(1): i_1(1));
else % multiple results
  i_1 = strfind(url,[''')">', strrep(my_pet, '+', ' '), '</a>']) - 1;
  if isempty(i_1); id = []; return; end
  i_0 = strfind(url(1:i_1), '''') + 1;
